...
build/wasm/busybox: source/busybox.tar.bz2 source/miniz.zip source/diff3prog.c
	...
	cat patch/001-wasm-Makefile.patch | patch -p1 -f
	PATH=$(CURDIR)/build/wasm:$$PATH emmake $(MAKE) -C build/wasm ARCH=em CROSS_COMPILE=em SKIP_STRIP=y
